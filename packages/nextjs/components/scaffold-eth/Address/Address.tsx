  ...
  if (!checkSumAddress) {
    return (
      <div className="flex items-center">
        <div
          className="flex-shrink-0 rounded-full animate-pulse bg-muted"
          style={{
  ...
  return (
    <div className="flex items-center flex-shrink-0">
      ...
      <div className="flex flex-col">
        {showSkeleton &&
          (isEnsNameLoading ? (
            <div className={cn("ml-5 animate-pulse rounded-lg font-bold bg-muted", textSizeMap[ensSize])}>
              <span className="invisible">{shortAddress}</span>
            </div>
          ) : (
            <span className={cn("ml-5 font-bold", textSizeMap[ensSize])}>
              <AddressLinkWrapper
        ...
        <div className="flex">
          <span className={cn("ml-5 font-normal", textSizeMap[addressSize])}>
            <AddressLinkWrapper
              disableAddressLink={disableAddressLink}
              blockExplorerAddressLink={blockExplorerAddressLink}
            >
