...
const Cert1: React.FC<DiplomaProps> = ({
  ...
  return (
    <div className="!container mx-auto p-0 m-0 border-[14px] border-[#D2B48C] min-w-[560px] bg-white text-black">
      ...
      <div className="text-[#D2B48C] text-center text-3xl mb-4">Certificado de Aprobación</div>

      <div className="mb-4 text-center">{institucion}, certifican por la presente que</div>

      <div className="w-3/5 mx-auto pb-2 mb-2 text-2xl italic text-center border-b-2 border-[#D2B48C]">
        <span className="font-bold">{nombre}</span>
        <br />
        <span className="flex justify-center text-lg not-italic">
          {address !== "" ? (
            <Address address={address} format="long" onlyEnsOrAddress size="lg" blockie={false} />
          ) : (
            pais !== "" && pais + " ID " + id
          )}
        </span>
      ...
      <div className="mb-4 text-center">
        ...
        <br />
        <span className="text-lg font-bold">{curso}</span>
