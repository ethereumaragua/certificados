...
const usePage = ({ params }: { params: { pais: number; id: number } }) => {
      ...
      for (let i = 0; i < datosYCertificadosUsuario[0].length; i++) {
        try {
          a = await certificadosContract?.read.mostrarCategoriaYCurso([
            BigInt(datosYCertificadosUsuario[0][i].categoria),
            BigInt(datosYCertificadosUsuario[0][i].curso),
          ]);
          if (a) {
            a.push({
              idCertificado: datosYCertificadosUsuario[0][i].idCertificado,
              usuario: datosYCertificadosUsuario[1],
            });
            arr.push(a);
          }
        } catch (error) {
  ...
  }, [datosYCertificadosUsuario, estaCertificado]);
  ...
  ({
    ...
    if (certs[index] && open)
      return (
        ...
        >
          <DialogContent className="min-w-[60%] overflow-y-auto max-h-screen">
            <VisuallyHidden>
