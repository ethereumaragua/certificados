  ...
  const estaCertificado = async() => {
    try{
      ...
      for(let i = 0; i < datosYCertificadosUsuarioAddress[0].length; i++) {
        try{
          a = await certificadosContract?.read.mostrarCategoriaYCurso([BigInt(datosYCertificadosUsuarioAddress[0][i].categoria), BigInt(datosYCertificadosUsuarioAddress[0][i].curso)]);
          if(a) {
            a.push({"idCertificado": datosYCertificadosUsuarioAddress[0][i].idCertificado, "usuario": datosYCertificadosUsuarioAddress[1]});
            arr.push(a);
          }
        } catch (error) {
          console.log(error);
...
const VerCertificado = memo(({ open, onClose, certSelected, setCertSelected, certs, params }: { open: boolean; onClose: () => void, certSelected: any, setCertSelected: any, certs: any, params: { address: string }}) => {
  ...
  if(certs[index] && open) return (
    <Dialog open={open} onOpenChange={() => {
      ...
      <DialogContent 
        className="min-w-[90%] lg:min-w-[75%] xl:min-w-[60%] overflow-y-auto max-h-screen"
      >
        <VisuallyHidden>
